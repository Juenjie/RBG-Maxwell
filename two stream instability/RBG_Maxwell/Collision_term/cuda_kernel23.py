...
with open(Collision_database.__path__[0]+'/selected_system.txt','r') as save_text:
    string = 'from ..Collision_database.selected_system.amplitude_square23 import Amplitude_square'.replace("selected_system",save_text.read()) 
    exec(string)
        ...
        if (k2x > -half_px[flavor[i_collision,1]] and k2x < half_px[flavor[i_collision,1]] and 
            ...
            if abs(k10+k20-k30-k40-p0) < 10**(-18):
                ...
                if Ja>1e-5 and momentum_product > 10**(-13):
                    ...
                    # f1*f2*tf3*tf4*tfp - tf1*tf2*tf3*f4*fp
                    # amplitude square in the formula is initial averaged and final summed
                    # so here we devide the degenacy factors here
...
4*k30*k40*k4x**2 + \
2*k40**2*k4x**2 + \
4*k1x*k4x**3 - 4*k3x*k4x**3 - \
k4x**4 - 4*k1y*k30**2*k4y - \
8*k1x*k1y*k3x*k4y + \
4*k1y*k3x**2*k4y - \
8*k1y**2*k3y*k4y + \
4*k30**2*k3y*k4y + \
...
4*k3x*k4x*k4y**2 - \
2*k4x**2*k4y**2 + \
4*k1y*k4y**3 - 4*k3y*k4y**3 - \
k4y**4 + 4*k30**2*k3z*k4z + \
8*k1x*k3x*k3z*k4z - \
4*k3x**2*k3z*k4z + \
8*k1y*k3y*k3z*k4z - \
4*k3y**2*k3z*k4z - \
