...
def obtain_particle_numbers(Nq, Ng, f, dx, dy, dz, dpx, dpy, dpz):
    '''
    ...
    return np.array([np.sum(f[i]) for i in range(7)])*phase_space_volume_element*np.array([Nq,Nq,Nq,Nq,Nq,Nq,Ng])
...
def obtain_particle_energy(Nq, Ng, f, dx, dy, dz, dpx, dpy, dpz,
                           px_grid_size,
                           py_grid_size,
                           pz_grid_size,
                           px_left_bound, 
    ...
    particle_energies = np.zeros(len(masses))
    ...
    for ipx in range(px_grid_size):
        ...
        for ipy in range(py_grid_size):
            ...
            for ipz in range(pz_grid_size):
                ...
                E = [math.sqrt(masses[i]**2+px**2+py**2+pz**2) for i in range(7)]
                
                particle_energies[6] += f[6,ipx,ipy,ipz]*E[6]
                
                for i_type in range(6):
                    particle_energies[i_type] += f[i_type,ipx,ipy,ipz]*E[i_type]
    
    return np.array([particle_energies[i] for i in range(7)])*phase_space_volume_element*np.array([Nq,Nq,Nq,Nq,Nq,Nq,Ng])
