...
def fun(x, para):
    ...
    component = para[1]
    sign = para[0]    
    return 0.5/(2*math.sinh(0.5*Y0))*math.exp(0.5*sign*y)*alphaEM/e*math.sinh(y)*rhoPM(xp,yp,zp,gamma,rho0,R0,d,RA,sign,impact_parameter)\
    *(thetapPM(xp,yp,zp,RA,gamma,-sign,impact_parameter))\
    *cross(0.,0.,1.,RPMx(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter),\
                    RPMy(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter),\
                    RPMz(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter),component)\
    *denominator(1,tt,xx,yy,zz,xp,yp,zp,y)
"""
...
def fun(x, para):
    ...
    component = para[1]
    sign = para[0]  
    return 0.5/(2*math.sinh(0.5*Y0))*math.exp(0.5*sign*y)*alphaEM/e*math.cosh(y)*rhoPM(xp,yp,zp,gamma,rho0,R0,d,RA,sign,impact_parameter)\
    *(thetapPM(xp,yp,zp,RA,gamma,-sign,impact_parameter))\
    *(RPMx(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter),\
      RPMy(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter),\
      RPMz(xp,yp,zp,xx,yy,zz,1,tt,v_variable,impact_parameter))[round(component)]\
    *denominator(1,tt,xx,yy,zz,xp,yp,zp,y)
"""

def EM_field_Participant(x_left_bound, y_left_bound, z_left_bound,\
    ...
    for i_GPU in range(len(n_time)):
        ...
        E = MCintegral_functional.remote(my_func = ESpectator, 
                                         domain = [[-RA-impact_parameter/2,RA+impact_parameter/2],[-RA,RA],[-RA/gamma,RA/gamma],[-Y_range,Y_range]], 
