...
#         print(self.scanned_vx, self.scanned_vz)
    ...
    def DT_VT(self, f_local, dt, current_time_step, processes):
        ...
        " 2024.3.12 -- Ming-yan Sun " 
        # change the boundary value 
        # start is (iy=0; iz=0; ipx=0; ipy=0; ipz=0)
        # end is (iy=ny-1; iz=nz-1; ipx=npx-1; ipy=npy-1; ipz=npz-1)
        
#         def change_term(f1, num_par, first_1, first_2, second_1, second_2):
#             if first_1 == first_2:
#                 f1.reshape([1, 2,\
#                       self.nx,self.ny,self.nz,self.npx,self.npy,self.npz])[0][num_par][first_1] = f1.reshape([1, 2,\
#                       self.nx,self.ny,self.nz,self.npx,self.npy,self.npz])[0][num_par][second_1]
#             if first_1 != first_2:    
#                 f1.reshape([1, 2,\
#                       self.nx,self.ny,self.nz,self.npx,self.npy,self.npz])[0][num_par][first_1:first_2] = f1.reshape([1, 2,\
#                       self.nx,self.ny,self.nz,self.npx,self.npy,self.npz])[0][num_par][second_1:second_2]
#             return f1
        
#         self.f1 = copy.deepcopy(self.f)
#         add_num = 50
        
#         self.f1 = change_term(self.f1, 0, 2*add_num,   2*add_num,   3*add_num,   3*add_num)
#         self.f1 = change_term(self.f1, 0, 3*add_num-1, 3*add_num-1, 2*add_num-1, 2*add_num-1)
#         self.f1 = change_term(self.f1, 1, 2*add_num,   2*add_num,   3*add_num,   3*add_num)
#         self.f1 = change_term(self.f1, 1, 3*add_num-1, 3*add_num-1, 2*add_num-1, 2*add_num-1)
 
#         # copy f to calculate the EM
         
#         self.f1 = change_term(self.f1, 0,         0,   add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 0,   add_num, 2*add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 0, 3*add_num, 4*add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 0, 4*add_num, 5*add_num, 2*add_num, 3*add_num)
        
#         self.f1 = change_term(self.f1, 1,         0,   add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 1,   add_num, 2*add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 1, 3*add_num, 4*add_num, 2*add_num, 3*add_num)
#         self.f1 = change_term(self.f1, 1, 4*add_num, 5*add_num, 2*add_num, 3*add_num)
        
#         self.f = copy.deepcopy(self.f1)
